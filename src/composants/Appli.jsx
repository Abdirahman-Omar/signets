...
export default function Appli() {
  ...
  function gererAjout(nom, couverture, couleur) {
    ...
    // Version avec Storage (Attention : ne fonctionnera pas avec toutes les URLs d'images)
    /*
      fetch(couverture).then(
      reponse => reponse.blob().then(
        image => storage.ref('couvertures').child(nom).put(image).then(
          refStorage => refStorage.ref.getDownloadURL().then(
            url => {
              const objDossier = {
                nom: nom,
                couverture: url,
